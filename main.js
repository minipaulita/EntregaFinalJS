...
function mapeo(productos) {
  ...
  for (let prod of productos) {
    if (prod.nombre === "Cuaderno") {
      $("#cards").append(`<div class="col-sm py-3">
...
$("#boton").on("click", () => {
  ...
  swal.fire("Compra Finalizada", `Su total es ${resultado}`, "success");
});
/*animaciones*/
$(`#logo`).click(() => {
...
$(`#buscar`).click(() => {
  let infoBusqueda = document
    ...
    .value.toLowerCase();
  almacenadosFiltro = almacenados.filter((prod) =>
    prod.nombre.toLowerCase().includes(infoBusqueda)
  );
